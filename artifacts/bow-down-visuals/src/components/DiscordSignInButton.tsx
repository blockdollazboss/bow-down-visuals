...
/** Discord "Clyde" mark (inline SVG, no asset dependency). */
...
export function DiscordSignInButton({ onClick, loading, label, testId }: DiscordSignInButtonProps) {
  return (
    <Button
      type="button"
      size="lg"
      ...
      onClick={onClick}
      disabled={loading}
      data-testid={testId ?? "btn-discord-signin"}
    >
      {loading ? (
        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ) : (
        <DiscordMark className="mr-2 h-5 w-5" />
      )}
      {label}
    </Button>
  );
