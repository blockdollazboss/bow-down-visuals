...
const DEFAULT_DESCRIPTION = "Just made this with @bowdownvisuals 🔥\n\n#musicvideo #contentcreator #aivideo";
const DESCRIPTION_LIMIT = 5000;

const PROGRESS_STAGES = [
  "Uploading to your Page…",
  "Facebook is processing the video…",
  "Finishing up…",
];
...
export function FacebookPostModal({ open, onClose, videoUrl, accounts }: Props) {
  ...
  async function generateAiDescription() {
    ...
    try {
      ...
      if (!res.ok || !Array.isArray(data.hooks) || data.hooks.length === 0) {
        throw new Error(data.message || "Description generation failed.");
      }
      setDescription(`${data.hooks[0]}\n\nMade with @bowdownvisuals 🔥\n#musicvideo #contentcreator #aivideo`);
      refreshProfile();
    } catch (err) {
  ...
  async function publish() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Couldn't publish to Facebook.");
    } finally {
      setPublishing(false);
    }
  }
      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-primary/25 bg-[#0b0b0d] p-6 space-y-5 shadow-[0_0_60px_rgba(218,165,32,0.15)]">
        ...
        {outOfCredits ? (
          ...
          <div className="space-y-4 text-center py-4">
            ...
            <p className="text-white font-bold">Your video is live on Facebook!</p>
            {permalink ? (
              <a
                href={permalink}
                target="_blank"
              ...
              <p className="text-xs text-white/40">Posted — check your Facebook Page.</p>
            )}
            <div>
          ...
          <>
            {usable.length > 1 && (
              <div>
                <label className="text-xs font-bold text-white/60 uppercase tracking-wider">Page</label>
                <select
                  value={accountId}
                ...
                >
                  {usable.map((a) => (
                    <option key={a.id} value={a.id} className="bg-black">
                      {a.pageName ?? a.username ?? "Facebook Page"}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div>
              ...
              />
              <p className="text-[11px] text-white/30 mt-1 text-right">{description.length}/{DESCRIPTION_LIMIT}</p>
            </div>
            ...
            <Button
              onClick={publish}
              disabled={publishing || !accountId || !description.trim()}
            ...
            {publishing && (
              <p className="text-[11px] text-white/30 text-center -mt-2">
                This can take a few minutes while Facebook processes the video. Don't close this window.
              </p>
            )}
          </>
        )}
