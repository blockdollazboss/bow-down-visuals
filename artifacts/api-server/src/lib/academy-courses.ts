/* ─── Creator Academy course catalog ────────────────────────────────────
   Static course metadata (free to browse — pure UI). Lesson *content* is
   generated on demand by GPT-6 (1 credit/lesson) via the academy routes,
   honoring the standing rule that everything on the site is AI-powered
   and every AI feature costs a fee.

   KEEP IN SYNC with the frontend copy at
   artifacts/bow-down-visuals/src/lib/academy-courses.ts (same ids, titles,
   lesson ids — the backend copy is the source of truth for validation). */
...
export function getAcademyLesson(
  courseId: string,
  lessonId: string,
): { course: AcademyCourse; lesson: AcademyLessonMeta } | undefined {
